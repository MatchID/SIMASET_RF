<?php
if($_SESSION[Kode_Menu_Aset] == '2'){
	
	if ($_GET[act] == "tambah_doc"){
?>
	<!-- Isi Halaman -->
                    ...
                    <form method=post action='' enctype='multipart/form-data' id="demo-form2" data-parsley-validate class="form-horizontal form-label-left">
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-md-5 col-sm-5 col-xs-12">
                          <select required="required" class="form-control" id="jenis" name=jenis>
							...
							<?php
                            $sql =mysql_query("select * from rf_aset_jenis_doc where ID_Jenis_Doc='2'order by Deskripsi_Doc asc");
                      ...
                      <table id="datatable-buttons" class='table table-striped table-bordered'>
						...
						echo "
                          <tr class='even pointer'>
                            ...
                            <td>$w[Nama_Unit]</td>
                            <td>$w[Nama_Kondisi]</td>
                            <td><input type='checkbox' name='lapor[]' value='$w[id]'> Laporkan</td>
                            </td>
                          </tr>
							";
                      ...
                      <div class="form-group">
                        <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                          <a href="?menu=doc_lapor"<button type="cancel" class="btn btn-primary">Batal</button></a>
...
<?php
		...
		if(isset($_POST['simpan'])){
			$perbaikan = mysql_query("select * from rf_aset_perbaikan ORDER BY ID_Perbaikan DESC LIMIT 1");
			$noper = mysql_fetch_array($perbaikan);
			
			$dokumen = mysql_query("select * from rf_aset_doc ORDER BY ID_Doc DESC LIMIT 1");
			$iddoc = mysql_fetch_array($dokumen);
			
			if ($noper[ID_Perbaikan] == null){
				$nop = 1;
			}else{
				$nop = $noper[ID_Perbaikan] + 1;
			}
			
			if ($iddoc[ID_Doc] == null){
				$dokx = 1;
			}else{
				$dokx = $iddoc[ID_Doc] + 1;
			}
				...
				$lapor = $_POST['lapor'];
				$jumlah_dipilih = count($lapor);
 
				for($x=0;$x<$jumlah_dipilih;$x++){
					echo $lapor[$x].", ".$nop." - ".$dokx." ..";
			...
			if(in_array($ext, $valid)){ 
					...
					if(move_uploaded_file($tmp, $folder.$filenya)){
							mysql_query("insert into rf_aset_perbaikan set 
							ID_Doc='$dokx',
							Tahun='$tahun' ");
							...
							Judul_Doc='$_POST[judul]' ");
								...
								BerhasilSimpan("?menu=doc_lapor",500);
				...
				FileSalah("?menu=doc_lapor&act=tambah_menu");
			}
		}
...
<div class='col-md-12 col-sm-12 col-xs-12'>
		...
		if(isset($_POST['edit'])){
			...
			if(in_array($ext, $valid)){ 
				...
				FileSalah("?menu=doc_lapor&act=tambah_doc");
			}
		}
		...
		BerhasilHapus("?menu=doc_baru_kep",500);
			...
			echo "<script>window.location = '?menu=doc_lapor'</script>";
...
<div class='col-md-12 col-sm-12 col-xs-12'>
                <div class='x_panel'>
                  <div class='x_title'>
                    <h2>Data Permohonan Perbaikan Fakultas <small><?php echo $_SESSION[Jabatan_Aset]." ".$_SESSION[Unit_Aset]; ?></small></h2>
                    <ul class='nav navbar-right panel_toolbox'>
                    </ul>
                    <div class='clearfix'></div>
                  </div>
                  ...
                  <div class='x_content'>
                    ...
                    <a class='btn btn-app' href='?menu=doc_lapor&act=tambah_doc'><i class='fa fa-plus btn btn-flat'> Permohonan</i></a>
			...
			$akun = mysql_fetch_array($akun2);
						...
						$q = mysql_query("SELECT *
										...
										left join rf_aset_jenis_doc on rf_aset_doc.ID_Jenis_Doc=rf_aset_jenis_doc.ID_Jenis_Doc where rf_aset_doc.ID_Unit='$akun[ID_Unit]' and rf_aset_doc.ID_Jenis_Doc='2'");
						while ($w=mysql_fetch_array($q)){
						
						if (($w[ID_Status_Doc] !== null) && ($w[ID_Status_Doc] !== '2')) {
							$status=$w[Deskripsi_Status_Doc];
							$act = "<a href='?menu=doc_lapor&act=download_doc&doc=$w[ID_Doc]'><center>Detail</a>";
						}elseif ($w[ID_Status_Doc] == '2'){
							$status="Sudah Di Acc Dekan";
							$act = "<a href='?menu=doc_lapor&act=download_doc&doc=$w[ID_Doc]'><center>Detail</a> | <a href='?menu=doc_lapor&act=pengajuan_doc&doc=$w[ID_Doc]'>Ajukan Ke Rektorat</a>";
						}else{
							$status = "Belum Di Proses";
							$act = "<a href='?menu=doc_lapor&act=download_doc&doc=$w[ID_Doc]'><center>Detail</a> | <a href='?menu=doc_lapor&act=edit_doc&doc=$w[ID_Doc]'>Perbaharui</a>";
						}
						
							$a = mysql_query("Select * from rf_aset_doc
							where ID_Unit=$w[ID_Unit] and ID_Jenis_Doc = '2'");
					...
					<?php
						echo "<tr><td colspan=7></td></tr>
                          <tr>
                            <td colspan=2><b>Total Permohonan Perbaikan</td>
                            <td colspan=6><b>$totala Buah</td>
                          </tr>
							";
