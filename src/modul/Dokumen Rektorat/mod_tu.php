                      ...
                      <div class="form-group">
                        ...
                        <div class="col-md-5 col-sm-5 col-xs-12">
                          <select required="required" class="form-control" id="status" name=status>
							...
							<?php
                            $sql =mysql_query("select * from rf_aset_status_doc where ID_Level='4'");
			...
			if(in_array($ext, $valid)){ 
					...
					if(move_uploaded_file($tmp, $folder.$filenya)){
							...
							mysql_query("Update rf_aset_doc set 
									ID_Doc='$_REQUEST[doc]',
									Disposisi_TU='$filenya',
									ID_Status_Doc='$_POST[status]'
									where ID_Doc='$_REQUEST[doc]'");
  
							BerhasilSimpan("?menu=doc_baru",500);
							histori("Disposisi Dokumen", "Disposisi Dokumen $_POST[judul] ke Rektor");
...
<div class='col-md-12 col-sm-12 col-xs-12'>
                <div class='x_panel'>
                  <div class='x_title'>
                    <h2>Data Permohonan TU Rektorat <small><?php echo $_SESSION[Jabatan_Aset]; ?></small></h2>
                    <ul class='nav navbar-right panel_toolbox'>
                    </ul>
                    <div class='clearfix'></div>
                  </div>
			...
			$akun = mysql_fetch_array($akun2);
						...
						$q = mysql_query("SELECT *
										...
										left join rf_aset_jenis_doc on rf_aset_doc.ID_Jenis_Doc=rf_aset_jenis_doc.ID_Jenis_Doc where rf_aset_doc.ID_Status_Doc='3'");
						while ($w=mysql_fetch_array($q)){
						
						if ($w[ID_Status_Doc] !== '3') {
